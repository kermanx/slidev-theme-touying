...
#show: zebraw.with(
  ...
  lang: false,
  numbering: true,
  numbering-font-args: (size: 12pt, fill: gray, baseline: 1.5pt),
)

#import "@preview/itemize:0.2.0" as el
#show: el.default-enum-list.with(
  enum-spacing: (1.2em, auto),
)

#show link: it => {
  if it.body.has("text") and it.body.text.starts-with("https") {
...
#show: dewdrop-theme.with(
  ...
  config-info(
    title: [常见 Linux 配置/使用问题],
    subtitle: [—— Some Miscellaneous Linux Configuration/Usage Issues],
    author: [吴擎宇],
    date: datetime.today(),
...
该部分主要参考 ustclug 201 文档 #footnote[https://201.ustclug.org/dev/ssh/#port-forwarding]。
...
该部分主要参考 ustclug 201 文档#footnote[https://201.ustclug.org/dev/ssh/#proxy]。
...
该部分主要参考 ustclug 201 文档#footnote[https://201.ustclug.org/dev/ssh/#jump-host]。
...
一提到安装 Docker，大部分人可能首先想到的是去 Docker 官网找教程下载 Docker Engine (Docker CE)，但是官网上的教程比较繁琐，其实 USTC Mirror Help#footnote[https://mirrors.ustc.edu.cn/help/docker-ce.html] 里就包含了简单的安装 Docker CE 的指令，直接复制粘贴运行就可以了：
...
除了阿里云、华为云这类国内大厂，目前#strike[活得比较好的]一个第三方镜像站是 DaoCloud#footnote[https://m.daocloud.io]。该镜像源采取白名单机制，只有列入白名单的镜像才能被加速访问。使用方法很简单，直接在你要拉取的仓库前面加上 `m.daocloud.io` 前缀就行了，比如：
...
通常来说，你可以安装好 Docker 的机器上直接执行以下指令来安装 Docker Rootless：
...
如果你觉得 Docker Rootless 配置代理怎么这么麻烦，那就对了，是时候使用 Podman 了，默认 Rootless 模式，只需要设置 `http_proxy` 和 `https_proxy` 环境变量就可以走代理，而且 Podman CLI 兼容 Docker CLI，你可以直接像这样直接把 docker 命令替换成 podman：
...
该部分主要参考 ustclug 201 文档 #footnote[https://201.ustclug.org/advanced/cuda/#nvidia-driver]。

- 如果你是 wsl 用户，想要在 wsl 内安装 Nvidia 驱动，那么你不需要在 wsl 里做任何事情，而是应该在 Windows 上安装 Nvidia 的官方驱动，一旦安装完成，wsl 内的 Nvidia 驱动也就安装好了。

- 如果是普通的 Linux 用户，那么你应该尽可能通过*包管理器*安装 Nvidia 驱动，而不是通过 Nvidia 官网下载的 .run 文件来安装，#strike[否则的你的系统很可能会当场/延迟爆炸]。
  - 通过包管理器也有两种方式，一种就是官方自行打包的驱动包，不需要添加源；一种是 Nvidia 自己打包的驱动包，位于 CUDA-Downloads#footnote[https://developer.nvidia.com/CUDA-downloads]，请通过添加源的方式安装，*不要下载运行 .run 文件*。
...
该部分主要参考 ustclug 201 文档 #footnote[https://201.ustclug.org/advanced/cuda/#install-cuda]。
...
如果你需要在 Docker 里使用 CUDA，那么你需要添加源并安装 Nvidia Container Toolkit，具体可以参考 Help - USTC Mirror#footnote[https://mirrors.ustc.edu.cn/help/libnvidia-container.html]。
...
- 最近，uv 也成为了一个非常受欢迎的 Python 包管理工具，它的虚拟环境是基于 venv 的，通过 pyproject.toml 管理依赖，并且提供了更为简洁方便的命令行界面和更快的安装速度。

- 除了 venv，conda 也是一个非常流行的包和环境管理工具，但是 conda 不仅能用来管理 Python 包，还能管理其他语言的依赖。
...
安装完成后，根据我校 Miniconda 对于 conda-forge 和 bioconda 源的镜像说明#footnote[https://mirrors.ustc.edu.cn/help/anaconda.html#miniconda]配置镜像。
      ...
      - envs/
        - \<environment-name\>
          - bin/
            ...
            - (e.g. python, gcc)
          - lib/
            - (all libraries, static or dynamic)
            - (e.g. libssl.so, libcc1.so)
      - pkgs/
...
+ 删除某个冲突的包，如果你发现实际上不需要
+ 重开一个新的环境，俗话说重装解决 99% 的问题
+ 重看 README 严格按照要求重配环境
+ 询问在同一个 repo 上 work 的同学怎么配出来的环境
+ 开个 issue 询问作者怎么配出来的环境
+ 清晰表述你所 work 的 repo，所使用的系统，配置的环境的步骤，给出尽可能详细的信息，在技术群里提问，也许有人有幸配成功了，皆大欢喜
+ #strike[没招了，还是选择等死吧]
...
- top: 最*基本*的实时进程信息查看工具，支持按照 CPU、内存、时间等多种方式排序和过滤进程，但是界面比较简陋，功能也比较有限。
- htop: 最*常用*的实时进程信息查看工具，支持交互式操作，可以按照 CPU、内存、时间等多种方式排序和过滤进程，还可以显示进程树、线程信息等。
  - 常驻集内存：$"RES" = "USS" + "SHR"$
  - 比例内存：$"PSS" = "USS" + sum_"lib" "SHR"_"lib" / n_"lib" $（更合理的内存计算）
- btop: htop 的*增强*版，提供了更为美观和丰富的界面，支持更多的系统资源监控，比如网络、磁盘等。
- ps: 任一时刻下查看所有进程信息的工具，可以通过各种选项来显示进程的各种信息，比如 `ps aux` 和 `ps -ef` 都可以显示所有进程的详细信息，搭配 grep、awk 等工具可以过滤出你想要的进程信息。

== 网络

- ss/netstat: 查看当前系统的*网络连接和端口占用情况*，支持显示 TCP、UDP、UNIX 等多种协议的连接信息，可以通过各种选项来过滤和排序连接，常用的有 `ss -tlnup` 和 `netstat -tlnup`，列举出当前系统所有监听的 TCP 和 UDP 端口以及对应的进程信息。
- nload: 实时监控*各网卡流量*的工具，直观展示每个网卡进出口流量的折线图，可以帮助你快速了解系统的网络流量情况。
- iftop: 实时监控*各网络连接的流量*的工具，可以显示每个连接的 IP、流量大小、流量速率（2s、10s、40s）等信息，支持按照流量大小排序连接。
- nethogs: 实时监控*各进程的网络流量*的工具，可以显示每个进程的 PID、名称、流量大小、流量速率等信息，支持按照流量大小排序进程。
- tcpdump: 基于命令行的*网络数据包嗅探*工具，可以捕获和分析网络上的数据包，支持各种协议的解析和过滤，配合 Wireshark 等工具可以进行更为深入的网络分析。

== I/O

- iostat: 实时监控*各磁盘 I/O 情况*的工具，可以显示每个磁盘的读写占用率、读写速率、平均等待时间等信息，帮助你了解系统的磁盘 I/O 性能，常用的有 `iostat -dx 1`，每秒刷新一次磁盘的详细 I/O 信息。
- iotop: 实时监控*各进程的 I/O 情况*，可以显示每个进程的 PID、名称、读写速率等信息，支持按照 I/O 占用率排序进程，常用的有 `iotop -o`，只显示正在进行 I/O 操作的进程。

= Basis

== Background

这一章主要讲一些 Linux 相关的基本知识，大部分取自于 ustclug 101 文档，但进行了大量删减，因此内容可能会比较杂乱，#strike[可能会想到哪讲到哪]。
...
- 一个典型的 x86_64 平台下的 Linux 系统从开机上电到进入 tty 登录界面的全启动流程
  - Firmware #sym.arrow Bootloader #sym.arrow Kernel #sym.arrow Init System (Systemd)
...
以上是对 ustclug 101 Chapter 1 #footnote[https://101.lug.ustc.edu.cn/Ch01/] 的一个非常简略的总结，更多细节请参阅原文档。
...
以上便是 apt 包管理器的基本使用方法了，更多更详细的用法可以参考 ustclug 101 Chapter 3#footnote[https://101.lug.ustc.edu.cn/Ch03/]。
...
- BIOS：系统自检 + 硬件初始化 #sym.arrow 从磁盘的主引导扇区（MBR，磁盘的第一个扇区，512 Bytes）加载 Bootloader 的第一阶段代码（446 bytes）到内存中，并将控制权交给 Bootloader。
- UEFI：系统自检 + 硬件初始化 #sym.arrow 从 EFI System Partition（ESP）中加载 Bootloader（通常是一个 .efi 文件）到内存中，并将控制权交给 Bootloader。
...
一个#strike[常见的]定制化的 GRUB 引导菜单的示例：

---

#align(center, image("mc-grub.png", width: 80%))
...
#grid(
  ...
  gutter: 1em,
  [
    - Linux 101#footnote[https://101.lug.ustc.edu.cn/]：ustclug 的 Linux 基础教程，内容非常全面，适合初学者入门，了解 Linux 的基本概念和使用方法。
    - Linux 201#footnote[https://201.ustclug.org/]：ustclug 的 Linux 进阶教程，内容更加深入，适合有一定基础的用户继续学习，或者遇到一些疑难杂症可能会有帮助。
    - USTC Mirror Help#footnote[https://mirrors.ustc.edu.cn/help/index.html]：USTC Mirror 的帮助文档，包含了各种软件的安装和配置镜像的方法。
  ],
  grid(
    columns: 1,
    align: center + horizon,
    gutter: .8em,
    text(fill: color.blue, font: "FandolHei")[「常见 Linux 配置/使用问题」活动反馈],
    image("feedback.png", height: 60%)
  )
)
